...
class Guiwiz(pyxbmct.AddonDialogWindow):
	...
	def main_buttons(self):
		self.BuildsButton= pyxbmct.Button('[COLOR %s][B]Buildovi[/B][/COLOR]' % MAIN_BUTTONS_TEXT, focusTexture=FBUTTON,noFocusTexture=BUTTON)
		self.placeControl(self.BuildsButton,-2 , 1 , 13, 8)
		self.connect(self.BuildsButton, lambda: self.BuildList())

		self.MaintButton = pyxbmct.Button('[COLOR %s][B]Održavanje[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
		self.placeControl(self.MaintButton, 2,11, 9, 8)
		self.connect(self.MaintButton, lambda: self.Maint())

		self.BackResButton = pyxbmct.Button('[COLOR %s][B]Pohrani/Vrati[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
		self.placeControl(self.BackResButton, 2, 21, 9, 8)
		self.connect(self.BackResButton, lambda: self.BackRes())

		self.ToolsButton = pyxbmct.Button('[COLOR %s][B]Alati[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
		self.placeControl(self.ToolsButton,2 , 31, 9, 8)
		self.connect(self.ToolsButton, lambda: self.Tools())

		self.InstallablesButton = pyxbmct.Button('[COLOR %s][B]Instalacije[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
		#self.placeControl(self.InstallablesButton,2 , 33, 9, 8)
		#self.connect(self.InstallablesButton, lambda: Installables())

		self.CloseButton = pyxbmct.Button('[COLOR %s][B]Zatvori[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=EXIT,noFocusTexture=BUTTON)
	...
	def list_update(self):
		...
		try:
			if self.getFocus() == buildlistmenu:
				...
				buildtextbox.setLabel('[COLOR %s]Odabran build: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR, name))
				vertextbox.setLabel('[COLOR %s]Verzija: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR, Bversion))
				koditextbox.setLabel('[COLOR %s]Kodi verzija: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,kodivers))
				desctextbox.setText('[COLOR %s]Opis builda: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,description))
		...
		try:
			if self.getFocus() == addonlist:
				...
				addtextbox.setLabel('[COLOR %s]Dodatak odabran:[COLOR %s] %s[/COLOR]' % (DES_T_COLOR,DESCOLOR,name))
				desctextboxA.setText('[COLOR %s]Opis dodatka:[COLOR %s] %s[/COLOR]' % (DES_T_COLOR,DESCOLOR,description))
		...
		try:
			if self.getFocus() == apklist:
				...
				name = re.compile('name="(.+?)"').findall(link)[pos]
				description = re.compile('description="(.+?)"').findall(link)[pos]
				apktextbox.setLabel('[COLOR %s]APK odabran:[COLOR %s] %s[/COLOR]' % (DES_T_COLOR,DESCOLOR,name))
				desctextboxAPK.setText('[COLOR %s]APK opis:[COLOR %s] %s[/COLOR]' % (DES_T_COLOR,DESCOLOR,description))
		...
		try:
			if self.getFocus() == emulist:
				...
				name = re.compile('name="(.+?)"').findall(link)[pos]
				description = re.compile('description="(.+?)"').findall(link)[pos]
				emutextbox.setLabel('[COLOR %s]EMU Odabran:[COLOR %s] %s[/COLOR]' % (DES_T_COLOR,DESCOLOR,name))
				desctextboxEMU.setText('[COLOR %s]EMU Opis:[COLOR %s] %s[/COLOR]' % (DES_T_COLOR,DESCOLOR,description))
		...
		try:
			if self.getFocus() == romlist:
				...
				name = re.compile('name="(.+?)"').findall(link)[pos]
				description = re.compile('description="(.+?)"').findall(link)[pos]
				romtextbox.setLabel('[COLOR %s]ROM PACK Odabran:[COLOR %s] %s[/COLOR]' % (DES_T_COLOR,DESCOLOR,name))
				desctextboxROM.setText('[COLOR %s]ROM PACK Opis:[COLOR %s] %s[/COLOR]' % (DES_T_COLOR,DESCOLOR,description))
	...
	def BuildList(self):
		...
		if not BUILDFILE == 'http://' and not BUILDFILE == '':
			...
			ThemeButton = pyxbmct.Button('[COLOR %s][B]Not Used[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
			...
			buildnamelabel  = pyxbmct.Label('[COLOR %s]Trenutno instaliran build: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,buildname))
			self.placeControl(buildnamelabel, 90, 2, 8, 25)
		
			buildversionlabel  = pyxbmct.Label('[COLOR %s]Trenutno instalirana verzija builda: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,buildversion))
			self.placeControl(buildversionlabel, 95, 2, 8, 20)
		
			skinname  = pyxbmct.Label('[COLOR %s]Ime kože koju koristi build: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,defaultskinname))
			self.placeControl(skinname, 100, 2, 11, 20)
		
			buildthemelabel  = pyxbmct.Label('[COLOR %s]Tema builda: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,buildtheme))
			self.placeControl(buildthemelabel, 90, 28, 8, 21)
		
			errorinstall  = pyxbmct.Label('[COLOR %s]Greške prilikom instalacije: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,errors))
			self.placeControl(errorinstall, 95, 28, 8, 20)
		
			lastupdatchk  = pyxbmct.Label('[COLOR %s]Zadnja provjera nadogradnje: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,lastbuildcheck))
