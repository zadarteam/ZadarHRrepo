...
def buildWizard(name, type, theme=None, over=False):
	...
	if type == 'gui':
		...
		if yes:
			...
			else: DIALOG.ok(ADDONTITLE, "[COLOR %s]Da biste spremili promjene, sada morate prisilno zatvoriti Kodi, pritisnite OK da biste prisilno zatvorili Kodi[/COLOR]" % COLOR2); wiz.killxbmc('true')
		else:
		...
		if yes_pressed:
			...
			if int(float(percent)) > 0:
				...
				DIALOG.ok(ADDONTITLE, "[COLOR %s]Da biste spremili promjene, sada morate prisilno zatvoriti Kodi, pritisnite OK da biste prisilno zatvorili Kodi[/COLOR]" % COLOR2); wiz.killxbmc('true')
			else:
				if isinstance(errors, unicode):
					error = error.encode('utf-8')
