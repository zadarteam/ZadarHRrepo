...
def normalize_mask(mask, is_micro):
    ...
    try:
        ...
        raise MaskError(
            'Invalid data mask "{0}". Must be an integer or a string which represents an integer value.'.format(mask))
    if is_micro:
        if not 0 <= mask < 4:
...
class Segments:
    ...
    def bit_length_with_overhead(self, version, eci, is_sa=False):
        ...
        # ECI overhead
        if eci:
            no_eci_indicators = sum([1 for segment in self.segments if
                                     segment.mode == consts.MODE_BYTE and segment.encoding != consts.DEFAULT_BYTE_ENCODING])
