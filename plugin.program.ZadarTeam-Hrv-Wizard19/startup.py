...
def installed_build_check():
    ...
    if not CONFIG.EXTRACT == '100' and CONFIG.EXTERROR > 0:
        ...
        yes = dialog.yesno(CONFIG.ADDONTITLE,
                           '[COLOR {0}]{1}[/COLOR] [COLOR {2}]was not installed correctly!\n'.format(CONFIG.COLOR1,
                                                                                                     CONFIG.COLOR2,
                                                                                                     CONFIG.BUILDNAME) +
                           ('Installed: [COLOR {0}]{1}[/COLOR] / '
                            'Error Count: [COLOR {2}]{3}[/COLOR]\n').format(CONFIG.COLOR1, CONFIG.EXTRACT,
                                                                            CONFIG.COLOR1,
                                                                            CONFIG.EXTERROR) +
        ...
        if not CONFIG.SKIN == defaults and not CONFIG.BUILDNAME == "":
            ...
            if not response:
                ...
                yes = dialog.yesno(CONFIG.ADDONTITLE,
                                   '{0} was not installed correctly!\n'.format(CONFIG.BUILDNAME) +
    ...
    if CONFIG.get_setting('installed') == 'true':
        ...
        thread = Thread(target=powitanie)
        thread.start()
        thread.join()
...
def powitanie():
    time.sleep(30)
    powitanie = requests.get("http://buntic-team.com/XML/textfile.txt").text
    dialog = xbmcgui.Dialog()
    dialog.textviewer("Powitanie", powitanie)
...
def save_trakt():
    ...
    if next_save <= current_time:
        ...
        logging.log(
            "[Trakt Data] Next Auto Save isn't until: {0} / TODAY is: {1}".format(CONFIG.get_setting('traktnextsave'),
...
def save_debrid():
    ...
    if next_save <= current_time:
        ...
        logging.log(
            "[Debrid Data] Next Auto Save isn't until: {0} / TODAY is: {1}".format(CONFIG.get_setting('debridnextsave'),
                                                                                   tools.get_date(formatted=True)),
            level=xbmc.LOGINFO)
...
def save_login():
    ...
    if next_save <= current_time:
        ...
        logging.log(
            "[Login Info] Next Auto Save isn't until: {0} / TODAY is: {1}".format(CONFIG.get_setting('loginnextsave'),
                                                                                  tools.get_date(formatted=True)),
            level=xbmc.LOGINFO)
...
def auto_clean():
    service = False
    days = [tools.get_date(formatted=True), tools.get_date(days=1, formatted=True),
            tools.get_date(days=3, formatted=True), tools.get_date(days=7, formatted=True),
