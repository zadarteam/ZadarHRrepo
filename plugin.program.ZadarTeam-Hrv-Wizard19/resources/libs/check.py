...
def check_build(name, ret):
    ...
    match = re.compile(
        'name="%s".+?ersion="(.+?)".+?rl="(.+?)".+?inor="(.+?)".+?ui="(.+?)".+?odi="(.+?)".+?heme="(.+?)".+?con="(.+?)".+?anart="(.+?)".+?review="(.+?)".+?dult="(.+?)".+?nfo="(.+?)".+?escription="(.+?)"' % name).findall(
        link)
    if len(match) > 0:
...
def check_info(name):
    link = name.replace('\n', '').replace('\r', '').replace('\t', '')
    match = re.compile(
        '.+?ame="(.+?)".+?xtracted="(.+?)".+?ipsize="(.+?)".+?kin="(.+?)".+?reated="(.+?)".+?rograms="(.+?)".+?ideo="(.+?)".+?usic="(.+?)".+?icture="(.+?)".+?epos="(.+?)".+?cripts="(.+?)".+?inaries="(.+?)"').findall(
        link)
    if len(match) > 0:
...
def check_theme(name, theme, ret):
    ...
    link = response.text.replace('\n', '').replace('\r', '').replace('\t', '')
    match = re.compile('name="{0}".+?rl="(.+?)".+?con="(.+?)".+?anart="(.+?)".+?dult=(.+?).+?escription="(.+?)"'.format(
        theme)).findall(link)
    if len(match) > 0:
    ...
    if len(match) > 0:
        ...
        if version > CONFIG.BUILDVERSION:
            if CONFIG.DISABLEUPDATE == 'false':
                logging.log(
                    "[Check Updates] [Installed Version: {0}] [Current Version: {1}] Opening Update Window".format(
                        CONFIG.BUILDVERSION, version))
                window.show_update_window(CONFIG.BUILDNAME, CONFIG.BUILDVERSION, version, icon, fanart)
            else:
                logging.log(
                    "[Check Updates] [Installed Version: {0}] [Current Version: {1}] Update Window Disabled".format(
                        CONFIG.BUILDVERSION, version))
        else:
            logging.log(
                "[Check Updates] [Installed Version: {0}] [Current Version: {1}]".format(CONFIG.BUILDVERSION, version))
    else:
    ...
    if not CONFIG.DEFAULTSKIN == '':
        if os.path.exists(os.path.join(CONFIG.ADDONS, CONFIG.DEFAULTSKIN)):
            if dialog.yesno(CONFIG.ADDONTITLE,
                            "[COLOR {0}]It seems that the skin has been set back to [COLOR {1}]{2}[/COLOR]".format(
...
def check_sources():
    ...
    if len(match) > 0:
        match2 = re.compile(
            '<source>.+?<name>(.+?)</name>.+?<path pathversion="1">(.+?)</path>.+?<allowsharing>(.+?)</allowsharing>.+?</source>').findall(
            match[0])
        progress_dialog.create(CONFIG.ADDONTITLE,
                               "[COLOR {0}]Scanning Sources for Broken links[/COLOR]".format(CONFIG.COLOR2))
        for name, path, sharing in match2:
            x += 1
            perc = int(tools.percentage(x, len(match2)))
            progress_dialog.update(perc,
                                   "\n[COLOR {0}]Checking [COLOR {1}]{2}[/COLOR]:[/COLOR]\n".format(CONFIG.COLOR2,
                                                                                                    CONFIG.COLOR1,
                                                                                                    name) +
        ...
        if len(bad) > 0:
            choice = dialog.yesno(CONFIG.ADDONTITLE,
                                  "[COLOR {0}]{1}[/COLOR][COLOR {2}] Source(s) have been found Broken\n".format(
                                      CONFIG.COLOR1, len(bad), CONFIG.COLOR2) +
                ...
                for name, path, sharing, working in bad:
                    ...
                    if dialog.yesno(CONFIG.ADDONTITLE,
                                    "[COLOR {0}]{1}[/COLOR][COLOR {2}] was reported as non working\n".format(
                                        CONFIG.COLOR1, name, CONFIG.COLOR2) +
                                    "[COLOR {0}]{1}[/COLOR]\n".format(CONFIG.COLOR1, path) +
            ...
            if len(remove) > 0:
                ...
                dialog.ok(CONFIG.ADDONTITLE,
                          ...
                          "Working: [COLOR {0}]{1}[/COLOR] | Kept: [COLOR {2}]{3}[/COLOR] | Removed: [COLOR {4}]{5}[/COLOR][/COLOR]".format(
                              CONFIG.COLOR2, CONFIG.COLOR1, alive, CONFIG.COLOR1, kept, CONFIG.COLOR1, removed))
            else:
...
def check_repos():
    ...
    while start < sleeptime:
        ...
        progress_dialog.update(perc,
                               '\n[COLOR {0}]Checking: [/COLOR][COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR2,
                                                                                              CONFIG.COLOR1, repolist[
                                                                                                                 start - 1].replace(
                                       CONFIG.ADDONS, '')[1:]))
        xbmc.sleep(1000)
    if progress_dialog.iscanceled():
