...
class AddonMenu:
                        ...
                        if section:
                            ...
                            if type.lower() == 'skin':
                                ...
                                directory.add_file(addonname, {'mode': 'addons', 'action': 'addon', 'name': plugin,
                                                               'addonurl': addonurl, 'repository': repository,
    ...
    def install_addon(self, plugin, urls, over=False):
        ...
        if not over:
            if xbmc.getCondVisibility('System.HasAddon({0})'.format(plugin)):
                install = self.dialog.yesno(CONFIG.ADDONTITLE,
                                            '[COLOR {0}]{1}[/COLOR] already installed. Would you like to reinstall it?'.format(
                                                CONFIG.COLOR1, plugin))
            else:
                install = self.dialog.yesno(CONFIG.ADDONTITLE,
                                            'Would you like to install [COLOR {0}]{1}[/COLOR]?'.format(CONFIG.COLOR1,
                                                                                                       plugin))
        else:
            install = True
    ...
    def install_addon_pack(self, name, url):
        ...
        if not response:
            logging.log_notify("[COLOR {0}]Addon Installer[/COLOR]".format(CONFIG.COLOR1),
                               '[COLOR {0}]{1}:[/COLOR] [COLOR {2}]Invalid Zip Url![/COLOR]'.format(CONFIG.COLOR1, name,
                                                                                                    CONFIG.COLOR2))
            return
