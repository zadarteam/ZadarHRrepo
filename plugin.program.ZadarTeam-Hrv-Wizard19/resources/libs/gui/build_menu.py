...
class BuildMenu:
    ...
    def _list_all(self, match, kodiv=None):
        ...
        for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
            ...
            if not CONFIG.DEVELOPER == 'true' and test.str_test(name):
                continue
            if 'vip' in name.lower() and CONFIG.ADDON.getSetting('ptw') == 'false': continue
            if 'vip' in name.lower(): name2 = "[COLOR gold]" + name + "[/COLOR]"
            if 'update' in name.lower() or 'aktualizacja' in name.lower(): name2 = "[COLOR orange]" + name + "[/COLOR]"
            if 'fresh' in name.lower(): name2 = "[COLOR lime]" + name + "[/COLOR]"

            if not kodiv or kodiv == int(float(kodi)):
                menu = self.create_install_menu(name)
                directory.add_dir('[{0}] {1} (v{2})'.format(float(kodi), name2, version),
                                  {'mode': 'viewbuild', 'name': name}, description=description, fanart=fanart,
                                  icon=icon, menu=menu, themeit=CONFIG.THEME2)
    ...
    def get_listing(self):
        ...
        match = re.compile(
            'name="(.+?)".+?ersion="(.+?)".+?rl="(.+?)".+?ui="(.+?)".+?odi="(.+?)".+?heme="(.+?)".+?con="(.+?)".+?anart="(.+?)".+?dult="(.+?)".+?escription="(.+?)"').findall(
            link)
        ...
        if len(match) >= 1:
            if CONFIG.SEPARATE == 'true':
                ...
                if count19 > 0:
                    ...
                    directory.add_file('[B]{0} Matrix Builds ({1})[/B]'.format(state, count19),
                                       {'mode': 'togglesetting',
    ...
    def view_build(self, name):
        ...
        for version, url, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
            ...
            if previewcheck:
                directory.add_file('View Video Preview', {'mode': 'buildpreview', 'name': name},
                                   description=description, fanart=fanart,
                                   icon=icon, themeit=CONFIG.THEME3)
            ...
            directory.add_file('Fresh install', {'mode': 'install', 'action': 'build', 'name': name, url: "fresh"}, description=description,
                               fanart=fanart,
                               icon=icon, themeit=CONFIG.THEME1)
            if "update" in build or "aktualizacja" in build:
                directory.add_file('Normal install', {'mode': 'install', 'action': 'build', 'name': name, url: "normal"}, description=description,
                                   fanart=fanart,
                                   icon=icon, themeit=CONFIG.THEME1)

            if guicheck:
                directory.add_file('Apply guiFix', {'mode': 'install', 'action': 'gui', 'name': name},
                                   description=description, fanart=fanart,
                                   icon=icon, themeit=CONFIG.THEME1)
            ...
            if themecheck:
                ...
                for themename, themeurl, themeicon, themefanart, themeadult, description in match:
                    ...
                    directory.add_file(themetitle,
                                       {'mode': 'install', 'action': 'theme', 'name': name, 'url': themename},
                                       description=description, fanart=themefanart,
    ...
    def build_info(self, name):
        ...
        if response:
            if check.check_build(name, 'url'):
                ...
                msg = "[COLOR {0}]Build Name:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1,
                                                                                         name)
                msg += "[COLOR {0}]Build Version:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2,
                                                                                             CONFIG.COLOR1, version)
                if themes:
                    msg += "[COLOR {0}]Build Theme(s):[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2,
                                                                                                  CONFIG.COLOR1,
                                                                                                  ', '.join(themes))
                msg += "[COLOR {0}]Kodi Version:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2,
                                                                                            CONFIG.COLOR1, kodi)
                msg += "[COLOR {0}]Adult Content:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2,
                                                                                             CONFIG.COLOR1, adult)
                msg += "[COLOR {0}]Description:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1,
                                                                                           description)

                if extend:
                    msg += "[COLOR {0}]Latest Update:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2,
                                                                                                     CONFIG.COLOR1,
                                                                                                     created)
                    msg += "[COLOR {0}]Extracted Size:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2,
                                                                                                      CONFIG.COLOR1,
                                                                                                      tools.convert_size(
                                                                                                          int(float(
                                                                                                              extracted))))
                    msg += "[COLOR {0}]Zip Size:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2,
                                                                                                CONFIG.COLOR1,
                                                                                                tools.convert_size(int(
                                                                                                    float(zipsize))))
                    msg += "[COLOR {0}]Skin Name:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2,
                                                                                                 CONFIG.COLOR1, skin)
                    msg += "[COLOR {0}]Programs:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2,
                                                                                                CONFIG.COLOR1, programs)
                    msg += "[COLOR {0}]Video:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2,
                                                                                             CONFIG.COLOR1, video)
                    msg += "[COLOR {0}]Music:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2,
                                                                                             CONFIG.COLOR1, music)
                    msg += "[COLOR {0}]Pictures:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2,
                                                                                                CONFIG.COLOR1, picture)
                    msg += "[COLOR {0}]Repositories:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2,
                                                                                                    CONFIG.COLOR1,
                                                                                                    repos)
                    msg += "[COLOR {0}]Scripts:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2,
                                                                                               CONFIG.COLOR1, scripts)
                    msg += "[COLOR {0}]Binaries:[/COLOR] [COLOR {1}]{2}[/COLOR]".format(CONFIG.COLOR2, CONFIG.COLOR1,
                                                                                        binaries)
    ...
    def create_install_menu(self, name):
        ...
        menu_items.append(
            (CONFIG.THEME2.format(name), 'RunAddon({0}, ?mode=viewbuild&name={1})'.format(CONFIG.ADDON_ID, buildname)))
        menu_items.append((CONFIG.THEME3.format('Fresh Install'),
                           'RunPlugin(plugin://{0}/?mode=install&name={1}&url=fresh)'.format(CONFIG.ADDON_ID,
                                                                                             buildname)))
        menu_items.append((CONFIG.THEME3.format('Normal Install'),
                           'RunPlugin(plugin://{0}/?mode=install&name={1}&url=normal)'.format(CONFIG.ADDON_ID,
                                                                                              buildname)))
        menu_items.append((CONFIG.THEME3.format('Apply guiFix'),
                           'RunPlugin(plugin://{0}/?mode=install&name={1}&url=gui)'.format(CONFIG.ADDON_ID, buildname)))
        menu_items.append((CONFIG.THEME3.format('Build Information'),
                           'RunPlugin(plugin://{0}/?mode=buildinfo&name={1})'.format(CONFIG.ADDON_ID, buildname)))
