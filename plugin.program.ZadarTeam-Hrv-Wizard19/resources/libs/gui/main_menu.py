...
class MainMenu:
    ...
    def get_listing(self):
        ...
        if len(CONFIG.BUILDNAME) > 0:
            ...
            if themefile:
                directory.add_file('None' if CONFIG.BUILDTHEME == "" else CONFIG.BUILDTHEME,
                                   {'mode': 'theme', 'name': CONFIG.BUILDNAME},
        ...
        if tools.open_url(CONFIG.ADDONFILE, check=True) or os.path.exists(
                os.path.join(CONFIG.ADDON_PATH, 'resources', 'text', 'addons.json')):
        ...
        directory.add_file('View Errors in Log: {0}'.format(errorsfound), {'mode': 'viewerrorlog'},
