...
def highlight_text(msg):
    msg = msg.replace('\n', '[NL]')
    matches = re.compile(
        "-->Python callback/script returned the following error<--(.+?)-->End of Python script error report<--").findall(
        msg)
    for item in matches:
        string = '-->Python callback/script returned the following error<--{0}-->End of Python script error report<--'.format(
            item)
        msg = msg.replace(string, '[COLOR red]{0}[/COLOR]'.format(string))
    msg = msg.replace('WARNING', '[COLOR yellow]WARNING[/COLOR]').replace('ERROR', '[COLOR red]ERROR[/COLOR]').replace(
...
def show_contact(msg=""):
    ...
    cw = ContactWindow("Contact.xml", CONFIG.ADDON_PATH, 'Default', title=CONFIG.ADDONTITLE,
                       fanart=CONFIG.CONTACTFANART,
...
def show_apk_warning(apk):
    ...
    xbmc.executebuiltin(
        'Skin.SetString(apkinstaller, Now that {0} has been downloaded[CR]Click install on the next window!)'.format(
            apk))
...
def show_build_prompt():
    class BuildPrompt(xbmcgui.WindowXMLDialog):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.msg = xbmcaddon.Addon().getLocalizedString(32242).format(
                CONFIG.ADDONTITLE, CONFIG.ADDONTITLE) #language translation test
        ...
        def do_build_menu(self):
            logging.log(
                "[Current Build Check] [User Selected: Open Build Menu] [Next Check: {0}]".format(CONFIG.BUILDCHECK),
                level=xbmc.LOGINFO)
        ...
        def do_ignore(self):
            logging.log(
                "[Current Build Check] [User Selected: Ignore Build Menu] [Next Check: {0}]".format(CONFIG.BUILDCHECK),
                level=xbmc.LOGINFO)
            CONFIG.set_setting('nextbuildcheck', tools.get_date(days=CONFIG.UPDATECHECK, formatted=True))
            CONFIG.set_setting('installed', 'ignored')
...
def show_update_window(name='Testing Window', current='1.0', new='1.1', icon=CONFIG.ADDON_ICON,
    ...
    class UpdateWindow(xbmcgui.WindowXMLDialog):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.msgupdate = "Update avaliable for installed build:\n[COLOR {0}]{1}[/COLOR]\n\nCurrent Version: v[COLOR {2}]{3}[/COLOR]\nLatest Version: v[COLOR {4}]{5}[/COLOR]\n\n[COLOR {6}]*Recommened: Fresh install[/COLOR]".format(
                CONFIG.COLOR1, self.name, CONFIG.COLOR1, self.current, CONFIG.COLOR1, self.new, CONFIG.COLOR1)
            self.msgcurrent = xbmcaddon.Addon().getLocalizedString(32038).format(
                CONFIG.COLOR1, self.name, CONFIG.COLOR1, self.current, CONFIG.COLOR1, self.new, CONFIG.COLOR1)
        ...
        def onInit(self):
            ...
            self.setProperty('dialog.textbox',
                             CONFIG.THEME2.format(self.msgupdate if current < new else self.msgcurrent))
        ...
        def do_fresh_install(self):
            logging.log(
                "[Check Updates] [Installed Version: {0}] [Current Version: {1}] [User Selected: Fresh Install build]".format(
                    CONFIG.BUILDVERSION, CONFIG.BUILDLATEST))
            logging.log(
                "[Check Updates] [Next Check: {0}]".format(tools.get_date(days=CONFIG.UPDATECHECK, formatted=True)))
            url = 'plugin://{0}/?mode=install&name={1}&action=fresh'.format(CONFIG.ADDON_ID,
        ...
        def do_normal_install(self):
            logging.log(
                "[Check Updates] [Installed Version: {0}] [Current Version: {1}] [User Selected: Normal Install build]".format(
                    CONFIG.BUILDVERSION, CONFIG.BUILDLATEST))
            logging.log(
                "[Check Updates] [Next Check: {0}]".format(tools.get_date(days=CONFIG.UPDATECHECK, formatted=True)))
            url = 'plugin://{0}/?mode=install&name={1}&action=normal'.format(CONFIG.ADDON_ID,
                                                                             quote_plus(CONFIG.BUILDNAME))
            xbmc.executebuiltin('RunPlugin({0})'.format(url))
            self.close()
        ...
        def do_ignore(self):
            logging.log(
                "[Check Updates] [Installed Version: {0}] [Current Version: {1}] [User Selected: Ignore {2} Days]".format(
                    CONFIG.BUILDVERSION, CONFIG.BUILDLATEST, CONFIG.UPDATECHECK))
            logging.log(
                "[Check Updates] [Next Check: {0}]".format(tools.get_date(days=CONFIG.UPDATECHECK, formatted=True)))
            self.close()
...
def show_notification(msg, test=False):
    ...
    xbmc.executebuiltin(
        'Skin.SetString(headertexttype, {0})'.format('true' if CONFIG.HEADERTYPE == 'Text' else 'false'))
    xbmc.executebuiltin(
        'Skin.SetString(headerimagetype, {0})'.format('true' if CONFIG.HEADERTYPE == 'Image' else 'false'))
