...
def wipe():
    ...
    progress_dialog.create(CONFIG.ADDONTITLE,
                           "[COLOR {0}]Calculating files and folders\n\n".format(
        ...
        for name in files:
            ...
            if name == 'sources.xml' and fold[-1] == 'userdata' and CONFIG.KEEPSOURCES == 'true':
                ...
                progress_dialog.update(int(tools.percentage(del_file, total_files)),
                                       '\n\n[COLOR {0}]File: [/COLOR][COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR2,
                                                                                                    CONFIG.COLOR1,
                                                                                                    name))
                try:
                    os.remove(os.path.join(root, name))
...
def fresh_start(install=None, over=False):
    ...
    if over:
        ...
        yes_pressed = dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]Do you wish to restore your\n".format(CONFIG.COLOR2) +
                                   "Kodi configuration to default settings\n" +
...
def install_apk(name, url):
    ...
    apk = os.path.basename(url).replace('\\', '').replace('/', '').replace(':', '').replace('*', '').replace('?',
                                                                                                             '').replace(
    ...
    if tools.platform() == 'android':
        ...
        if os.path.exists(lib):
            redownload = dialog.yesno(CONFIG.ADDONTITLE,
                                      '[COLOR {}]{}[/COLOR] already exists. Would you like to redownload it?'.format(
                                          CONFIG.COLOR1, apk),
        ...
        if yes or redownload:
            ...
            progress_dialog.create(CONFIG.ADDONTITLE,
                                   '[COLOR {0}][B]Downloading:[/B][/COLOR] [COLOR {1}]{2}[/COLOR]\n\n'.format(
                                       CONFIG.COLOR2, CONFIG.COLOR1, apk) + 'Please Wait')
        ...
        dialog.ok(CONFIG.ADDONTITLE,
                  '[COLOR {}]{}[/COLOR] downloaded to [COLOR {}]{}[/COLOR]. If installation doesn\'t start by itself, navigate to that location to install the APK.'.format(
                      CONFIG.COLOR1, apk, CONFIG.COLOR1, path))
