...
def log(msg, level=xbmc.LOGDEBUG):
    ...
    if CONFIG.ENABLEWIZLOG == 'true':
        ...
        if CONFIG.CLEANWIZLOG == 'true' and time.mktime(
                time.strptime(lastcheck, "%Y-%m-%d %H:%M:%S")) <= tools.get_date():
            check_log()
...
def upload_log():
    ...
    for item in files:
        ...
        if succes:
            ...
            if succes:
                msg = xbmcaddon.Addon().getLocalizedString(32243).format(
...
def read_log(path):
    try:
        ...
        content = lf.readBytes().decode("utf-8", errors="ignore")
        if content:
            return True, content
        else:
            log('file is empty')
...
def view_log_file():
    ...
    choices = []
    logfiles = {'mainlog': "View {0}".format(os.path.basename(mainlog)),
                'oldlog': "View {0}".format(os.path.basename(oldlog)),
                'wizlog': "View {0}".format(os.path.basename(wizlog))}
