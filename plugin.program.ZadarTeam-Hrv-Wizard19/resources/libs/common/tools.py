...
def read_from_file(file, mode='rb'):
    f = open(file, mode)
...
def ensure_folders(folder=None):
    ...
    try:
        ...
        dialog.ok(CONFIG.ADDONTITLE,
                  "[COLOR {0}]Error creating add-on directories:[/COLOR]\n".format(CONFIG.COLOR2) +
...
def parse_dom(html, name=u"", attrs={}, ret=False):
    ...
    for item in html:
        ...
        lst = []
        for key in attrs:
            lst2 = re.compile('(<' + name + '[^>]*?(?:' + key + '=[\'"]' + attrs[key] + '[\'"].*?>))',
                              re.M | re.S).findall(item)
            if len(lst2) == 0 and attrs[key].find(" ") == -1:
                lst2 = re.compile('(<' + name + '[^>]*?(?:' + key + '=' + attrs[key] + '.*?>))', re.M | re.S).findall(
                    item)
...
def convert_special(url, over=False):
    ...
    progress_dialog.create(CONFIG.ADDONTITLE,
                           "[COLOR {0}]Changing Physical Paths To Special\n\n".format(CONFIG.COLOR2) +
                           "Please Wait[/COLOR]")
        ...
        for file in files:
            ...
            if file.endswith(".xml") or file.endswith(".hash") or file.endswith("properies"):
                progress_dialog.update(perc,
                                       "[COLOR {0}]Scanning: [COLOR {1}]{2}[/COLOR]\n".format(CONFIG.COLOR2,
                                                                                              CONFIG.COLOR1,
                                                                                              root.replace(CONFIG.HOME,
                                                                                                           '')) +
                                       "[COLOR {0}]{1}[/COLOR]\n".format(CONFIG.COLOR1, file) +
                                       "Please Wait[/COLOR]")
                ...
                b = a.replace(CONFIG.HOME, 'special://home/').replace(encodedpath, 'special://home/').replace(
                    encodedpath2, 'special://home/')
...
def reload_fix(default=None):
    ...
    dialog.ok(CONFIG.ADDONTITLE,
              "[COLOR {0}]WARNING: Sometimes Reloading the Profile causes Kodi to crash. While Kodi is Reloading the Profile Please Do Not Press Any Buttons![/COLOR]".format(
                  CONFIG.COLOR2))
...
def ascii_check(use=None, over=False):
    ...
    if use is None:
        ...
        if over:
            ...
            yes = dialog.yesno(CONFIG.ADDONTITLE,
                               '[COLOR {0}]Do you want to [COLOR {1}]delete[/COLOR] all filenames with special characters or would you rather just [COLOR {2}]scan and view[/COLOR] the results in the log?[/COLOR]'.format(
    ...
    for base, dirs, files in os.walk(source):
        ...
        for file in files:
            ...
            progress_dialog.update(prog2, "[COLOR {0}]Checking for non ASCII files\n".format(CONFIG.COLOR2) +
                                   '[COLOR {0}]{1}[/COLOR]\n'.format(CONFIG.COLOR1, file) + 'Please Wait[/COLOR]')
            try:
                file.encode('ascii')
    ...
    if total > 0:
        ...
        if yes:
            if use:
                logging.log_notify(CONFIG.ADDONTITLE,
                                   "[COLOR {0}]ASCII Check: {1} Removed / {2} Failed.[/COLOR]".format(CONFIG.COLOR2, f1,
                                                                                                      f2))
            else:
            ...
            window.show_text_box("Viewing Found ASCII Files",
                                 "[COLOR yellow][B]{0} Files Found:[/B][/COLOR]\n {1}".format(f1, msg))
    else:
        logging.log_notify(CONFIG.ADDONTITLE,
    ...
    if _is_url(url):
        try:
            ...
            if response.status_code < 300:
                logging.log("URL check passed for {0}: Status code [{1}]".format(url, response.status_code),
                            level=xbmc.LOGDEBUG)
                return True
            elif response.status_code < 400:
                logging.log(
                    "URL check redirected from {0} to {1}: Status code [{2}]".format(url, response.headers['Location'],
                                                                                     response.status_code),
                    level=xbmc.LOGDEBUG)
                return _check_url(response.headers['Location'])
            elif response.status_code == 401:
                logging.log("URL requires authentication for {0}: Status code [{1}]".format(url, response.status_code),
                            level=xbmc.LOGDEBUG)
                return 'auth'
            else:
                logging.log("URL check failed for {0}: Status code [{1}]".format(url, response.status_code),
                            level=xbmc.LOGDEBUG)
                return False
        except Exception as e:
...
def open_url(url, stream=False, check=False, cred=None, count=0):
    ...
    if not valid:
        ...
        if response.status_code == 401:
            retry = dialog.yesno(CONFIG.ADDONTITLE,
