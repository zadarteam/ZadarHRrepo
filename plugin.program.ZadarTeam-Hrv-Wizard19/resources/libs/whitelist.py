...
def whitelist(do):
    ...
    if do == 'edit':
        ...
        while choice not in [-1, 0]:
            choice = dialog.select("{0}: Select the add-ons you wish to the whitelist.".format(CONFIG.ADDONTITLE),
                                   tempaddonnames)
            if choice == -1:
                break
            elif choice == 0:
        ...
        if len(list) > 0:
            ...
            for item in list:
                try:
                    ...
                    logging.log(str(e))
                msg += "[COLOR {0}]{1}[/COLOR] [COLOR {2}]\"{3}\"[/COLOR][CR]".format(CONFIG.COLOR1, name,
                                                                                      CONFIG.COLOR2, id)
        ...
        source = dialog.browse(3,
                               '[COLOR {0}]Select where you wish to export the whitelist file[/COLOR]'.format(
                                   CONFIG.COLOR2),
                               'files', '.txt', False, False, CONFIG.HOME)
        logging.log(str(source))
        try:
            ...
            dialog.ok(CONFIG.ADDONTITLE,
                      "[COLOR {0}]Whitelist has been exported to:[/COLOR]\n".format(CONFIG.COLOR2) +
            ...
            if not dialog.yesno(CONFIG.ADDONTITLE,
                                "[COLOR {0}]The location you selected isn\'t writable would you like to select another one?[/COLOR]".format(
                                    CONFIG.COLOR2),
        ...
        if not dialog.yesno(CONFIG.ADDONTITLE,
                            "[COLOR {0}]Are you sure you want to clear your whitelist?\n".format(CONFIG.COLOR2) +
