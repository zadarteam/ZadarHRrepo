...
class Restore:
    ...
    def _from_file(self, file, loc):
        ...
        if not self.external:
            try:
                ...
                self.progress_dialog.update(0, '[COLOR {0}]Unable to read zip file from current location.\n'.format(
    ...
    def _view_errors(self, percent, errors, error, file):
        if int(errors) >= 1:
            if self.dialog.yesno(CONFIG.ADDONTITLE,
                                 '[COLOR {0}][COLOR {1}]{2}[/COLOR]\n'.format(CONFIG.COLOR2, CONFIG.COLOR1, file) +
    ...
    def choose(self, location):
        ...
        self.progress_dialog.create(CONFIG.ADDONTITLE, '[COLOR {0}]Installing {1} Backup\n'.format(
            CONFIG.COLOR2, external) + '\nPlease Wait[/COLOR]')
