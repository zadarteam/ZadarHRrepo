...
def addon_database(addon=None, state=1, array=False):
    ...
    try:
        if not array:
            textexe.execute('INSERT or IGNORE into installed (addonID , enabled, installDate) VALUES (?,?,?)',
                            (addon, state, installedtime,))
            textexe.execute('UPDATE installed SET enabled = ? WHERE addonID = ? ', (state, addon,))
        else:
            for item in addon:
                textexe.execute('INSERT or IGNORE into installed (addonID , enabled, installDate) VALUES (?,?,?)',
                                (item, state, installedtime,))
    ...
    for folder in sorted(addonlist, key=lambda x: x):
        ...
        if os.path.exists(addonxml):
            ...
            try:
                ...
                try:
                    logging.log("{0} was disabled".format(addonid))
...
def toggle_addon(id, value, over=None):
    ...
    query = '{{"jsonrpc":"2.0", "method":"Addons.SetAddonEnabled","params":{{"addonid":"{0}","enabled":{1}}}, "id":1}}'.format(
        addonid, value)
    ...
    if 'error' in response and over is None:
        ...
        dialog.ok(CONFIG.ADDONTITLE,
                  "[COLOR {0}]Error {1} [COLOR {2}]{3}[/COLOR]\n".format(CONFIG.COLOR2, v, CONFIG.COLOR1, id) +
    ...
    if os.path.exists(dep):
        ...
        for depends in match:
            if 'xbmc.python' not in depends:
                ...
                if dp is not None:
                    dp.update("",
                              "Checking Dependency [COLOR yellow]{0}[/COLOR] for [COLOR yellow]{1}[/COLOR]".format(
                                  depends, name),
                              "")
...
def find_binary_addons(addon='all'):
    ...
    if addon == 'all':
        ...
        dialog.ok(CONFIG.ADDONTITLE,
                  "[COLOR {0}]Found [COLOR {1}]{2}[/COLOR] platform-specific addons installed:[/COLOR]\n".format(
                      CONFIG.COLOR2, CONFIG.COLOR1, len(addonnames)) +
                  "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, addonnames))
        ...
        if os.path.exists(xml):
            ...
            content = open(xml, "rb").read()
            addonid = xml.split("\\")[-2]
            addonname = re.findall('''name=\"(.*?)"''', content.decode("utf-8", errors="ignore"))[0]

            if 'library_' in content.decode("utf-8", errors="ignore"):
