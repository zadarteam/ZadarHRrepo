                ...
                if file_count > 0:
                    ...
                    if over:
                        ...
                        yes = dialog.yesno("[COLOR {0}]Delete Package Files".format(CONFIG.COLOR2),
                                           "[COLOR {0}]{1}[/COLOR] files found / [COLOR {2}]{3}[/COLOR] in size.\n".format(
...
def clear_packages_startup():
    ...
    if os.path.exists(CONFIG.PACKAGES):
        ...
        try:
            ...
            if file_count > 0:
                logging.log_notify(CONFIG.ADDONTITLE,
                                   '[COLOR {0}]Clear Packages: Success: {1}[/COLOR]'.format(CONFIG.COLOR2,
                                                                                            tools.convert_size(
                                                                                                cleanupsize)))
            else:
                logging.log_notify(CONFIG.ADDONTITLE,
                ...
                if file_count > 0:
                    ...
                    for d in dirs:
                        try:
                            ...
                            delfiles += 1
                            logging.log(
                                "[Success] cleared {0} files from {1}".format(str(file_count), os.path.join(item, d)),
                                level=xbmc.LOGINFO)
                        except:
...
def old_thumbs():
    ...
    if len(images) > 0:
        logging.log_notify(CONFIG.ADDONTITLE,
                           '[COLOR {0}]Clear Thumbs: {1} Files / {2} MB[/COLOR]!'.format(CONFIG.COLOR2,
                                                                                         str(len(images)), removed))
    else:
        logging.log_notify(CONFIG.ADDONTITLE,
...
def clear_crash():
    ...
    if len(files) > 0:
        ...
        if dialog.yesno(CONFIG.ADDONTITLE,
                        '[COLOR {0}]Would you like to delete the Crash logs?\n'.format(CONFIG.COLOR2) +
...
def toggle_cache(state):
    cachelist = ['includevideo', 'includeall', 'includeexodusredux', 'includegaia', 'includenumbers', 'includescrubs',
                 'includeseren', 'includethecrew', 'includevenom']
    titlelist = ['Include Video Addons', 'Include All Addons', 'Include Exodus Redux', 'Include Gaia',
                 'Include NuMb3r5', 'Include Scrubs v2', 'Include Seren', 'Include THE CREW', 'Include Venom']
        ...
        if state not in ['includevideo', 'includeall'] and CONFIG.get_setting('includeall') == 'true':
            try:
                ...
                dialog.ok(CONFIG.ADDONTITLE,
                          "[COLOR {0}]You will need to turn off [COLOR {1}]Include All Addons[/COLOR] to disable[/COLOR] [COLOR {2}]{3}[/COLOR]".format(
                              CONFIG.COLOR2, CONFIG.COLOR1, CONFIG.COLOR1, item))
            except:
...
def remove_addon(addon, name, over=False, data=True):
    ...
    if over:
        ...
        yes = dialog.yesno(CONFIG.ADDONTITLE,
                           '[COLOR {0}]Are you sure you want to delete the add-on:\n'.format(CONFIG.COLOR2) +
                           'Name: [COLOR {0}]{1}[/COLOR]\n'.format(CONFIG.COLOR1, name) +
...
def remove_addon_data(addon):
    ...
    if addon == 'all':  # clear ALL addon data
        if dialog.yesno(CONFIG.ADDONTITLE,
                        '[COLOR {0}]Would you like to remove [COLOR {1}]ALL[/COLOR] addon data stored in your userdata folder?[/COLOR]'.format(
        ...
        if dialog.yesno(CONFIG.ADDONTITLE,
                        '[COLOR {0}]Would you like to remove [COLOR {1}]ALL[/COLOR] addon data stored in your userdata folder for uninstalled addons?[/COLOR]'.format(
                            CONFIG.COLOR2, CONFIG.COLOR1),
                        yeslabel='[B][COLOR springgreen]Remove Data[/COLOR][/B]',
                        nolabel='[B][COLOR red]Don\'t Remove[/COLOR][/B]'):
        ...
        if dialog.yesno(CONFIG.ADDONTITLE,
                        '[COLOR {0}]Would you like to remove [COLOR {1}]ALL[/COLOR] empty addon data folders in your userdata folder?[/COLOR]'.format(
                            CONFIG.COLOR2, CONFIG.COLOR1),
                        yeslabel='[B][COLOR springgreen]Remove Data[/COLOR][/B]',
                        nolabel='[B][COLOR red]Don\'t Remove[/COLOR][/B]'):
