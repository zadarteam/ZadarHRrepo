...
def cleanup_backup():
    ...
    if selected == -1:
        ...
        if dialog.yesno(CONFIG.ADDONTITLE,
                        "[COLOR {0}]Would you like to clean up all items in your 'My_Builds' folder?[/COLOR]\n".format(
                            CONFIG.COLOR2) +
        ...
        if dialog.yesno(CONFIG.ADDONTITLE,
                        "[COLOR {0}]Would you like to remove [COLOR {1}]{2}[/COLOR] from the 'My_Builds' folder?[/COLOR]\n".format(
...
class Backup:
    ...
    def backup_addon_pack(self, name=""):
        if self.dialog.yesno(CONFIG.ADDONTITLE,
            ...
            self.progress_dialog.create(CONFIG.ADDONTITLE,
                                        '[COLOR {0}][B]Creating Zip File:[/B][/COLOR]\n\n'.format(CONFIG.COLOR2) +
                                        'Please Wait')
            if len(selected) > 0:
                added = []
    ...
    def backup_build(self, name=""):
        if self.dialog.yesno(CONFIG.ADDONTITLE,
            ...
            if not self.dialog.yesno(CONFIG.ADDONTITLE,
                                     "[COLOR {0}]Do you want to include your addon_data folder?\n".format(
                                         CONFIG.COLOR2) +
                                     "This contains [COLOR {0}]ALL[/COLOR] add-on settings including passwords but may also contain important information such as skin shortcuts. We recommend [COLOR {0}]MANUALLY[/COLOR] removing the addon_data folders that aren\'t required.\n".format(
            ...
            self.progress_dialog.create(
                CONFIG.ADDONTITLE + "[COLOR {0}]: Creating Zip[/COLOR]\n".format(CONFIG.COLOR2) +
                "[COLOR {0}]Creating backup zip\n\n".format(CONFIG.COLOR2) +
                "Please Wait...[/COLOR]")
                ...
                for file in files:
                    try:
                        ...
                        self.progress_dialog.update(int(progress),
                                                    '[COLOR {0}]Creating backup zip: [COLOR {1}]{2}[/COLOR] / [COLOR {3}]{4}[/COLOR]\n'.format(
    ...
    def backup_gui(self, name=""):
        ...
        if os.path.exists(CONFIG.GUISETTINGS):
            ...
            try:
                ...
                for fold in match:
                    ...
                    if fd not in ['skin.confluence', 'skin.estuary', 'skin.estouchy']:
                        if self.dialog.yesno(CONFIG.ADDONTITLE,
                                             "[COLOR {0}]Would you like to add the following skin folder to the GUI Fix Zip File?[/COLOR]\n".format(
                                                 CONFIG.COLOR2) +
                                             "[COLOR {0}]{1}[/COLOR]\n".format(CONFIG.COLOR1, fd),
        ...
        if name == "":
            self.dialog.ok(CONFIG.ADDONTITLE, "[COLOR {0}]GUI Fix backup successful:[/COLOR]\n".format(CONFIG.COLOR2) +
            ...
            if not CONFIG.SKIN not in ['skin.confluence', 'skin.estuary', 'skin.estouchy']:
                ...
                if len(match2) > 1:
                    if self.dialog.yesno(
                            CONFIG.ADDONTITLE + '[COLOR {0}]: Theme Backup[/COLOR]\n'.format(CONFIG.COLOR2) +
                            "[COLOR {0}]Would you like to go through the Texture Files for?[/COLOR]\n".format(
                                CONFIG.COLOR2) +
                        ...
                        for xbt in match2:
                            if self.dialog.yesno(
                                    CONFIG.ADDONTITLE + '[COLOR {0}]: Theme Backup[/COLOR]\n'.format(CONFIG.COLOR2) +
                                    "[COLOR {0}]Would you like to add the Texture File [COLOR {1}]{2}[/COLOR]?\n".format(
                ...
                if os.path.exists(ad_skin):
                    if self.dialog.yesno(
                            CONFIG.ADDONTITLE + '[COLOR {0}]: Theme Backup[/COLOR]\n'.format(CONFIG.COLOR2) +
                            "[COLOR {0}]Would you like to go add the [COLOR {1}]settings.xml[/COLOR] in [COLOR {2}]/addon_data/[/COLOR] for?\n".format(
            ...
            for base, dirs, files in os.walk(CONFIG.ADDON_DATA):
                ...
                for file in files:
                    try:
                        ...
                        progress = tools.percentage(for_progress, N_ITEM)
                        self.progress_dialog.update(int(progress),
                                                    '[COLOR {0}]Creating back up zip: [COLOR{1}]{2}[/COLOR] / [COLOR{3}]{4}[/COLOR]\n'.format(
                                                        CONFIG.COLOR2, CONFIG.COLOR1, for_progress, CONFIG.COLOR1,
                                                        N_ITEM) +
                                                    '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, file))
                        fn = os.path.join(base, file)
