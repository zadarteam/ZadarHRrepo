...
TRAKTID = {
    'gaia': {
        ...
        'data': ['accounts.informants.trakt.user', 'accounts.informants.trakt.refresh',
                 'accounts.informants.trakt.token'],
    ...
    if who == 'all':
        for log in ORDER:
            if os.path.exists(TRAKTID[log]['path']):
                ...
                logging.log(
                    '[Trakt Data] {0}({1}) is not installed'.format(TRAKTID[log]['name'], TRAKTID[log]['plugin']),
                    level=xbmc.LOGERROR)
        ...
        if os.path.exists(TRAKTID[who]['path']):
            ...
            if not u or u == '':
                ...
                if dialog.yesno(CONFIG.ADDONTITLE,
                                "[COLOR {0}]Would you like to save the [COLOR {1}]Trakt Data[/COLOR] for [COLOR {2}]{3}[/COLOR]?\n".format(
