...
myTOKEN = addon.getSetting('pers_apiKey')
...
def listBeatportVideos(type, url, limit):
	...
	for i in range(1,len(spl),1):
		...
		if filtered:
			continue
		if type == 'play':
			url = '{0}?mode=playYTByTitle&url={1}'.format(sys.argv[0], quote_plus(firstTitle.replace(' - ', ' ')))
		else:
			url = firstTitle
...
def listItunesVideos(type, genreID, limit):
	...
	for item in response['feed']['entry']:
		...
		if filtered:
			continue
		if type == 'play':
			url = '{0}?mode=playYTByTitle&url={1}'.format(sys.argv[0], quote_plus(shortenTitle.replace(' - ', ' ')))
		else:
			url = shortenTitle
...
def getYoutubeId(title):
	...
	videoBest = False
	movieID = []
	content = cache('https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&maxResults=5&order=relevance&q={0}&key={1}'.format(title, myTOKEN), 1)
	response = json.loads(content)
